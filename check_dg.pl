...
open DST, "> ./dg_check/$tile_name.xls" or die;
...
open SRC, "./dg_check/$tile_name.xls" or die;
open DST, "> ./dg_check/$tile_name.final.xls" or die;
@lines = <SRC>;
@lines = sort @lines;
...
sub waive {
 ...
 while (<SRC>) {
  ...
  if($line =~ /^\s+(LINT-\d+):(.*)/) {
    ...
    $content = $2;
    print DST "dc WAIVE dixi all $tag1 .*$content.*\n";
    } elsif ($line =~ /^\s+(DONT_USE):Found dont_use cell (\S+)/) {
    $tag1 = $1;
    $content = $2;
    print DST "dc WAIVE dixi all $tag1 .*$content.*\n";
    } elsif ($line =~ /^\s+(.*?):/) {
